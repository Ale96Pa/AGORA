...
const TechnicalAnalysis = ({ width = 1000, height = 500, globalFilterTrigger, refreshTrigger }) => {
  ...
  // Function to build and sort distributions
  const buildDistribution = (data, key) => {
    const countMap = {};
    data.forEach(trace => {
      const value = trace[key] == null ? '?' : trace[key];
      countMap[value] = (countMap[value] || 0) + 1;
    });

    const distribution = Object.keys(countMap).map(value => ({
      value,
      count: countMap[value],
    }));

    // Sort numerically for impact, urgency, and priority
    if (key === 'impact' || key === 'urgency' || key === 'priority') {
      const predefinedOrder = key === 'priority' ? ['1', '2', '3', '4'] : ['1', '2', '3'];
      return distribution.sort((a, b) => predefinedOrder.indexOf(a.value) - predefinedOrder.indexOf(b.value));
    }

    // Default sorting by count (descending) for other keys
  ...
  useEffect(() => {
    const fetchTechnicalAttributes = async () => {
      ...
      try {
        ...
        setCategoryDistribution(buildDistribution(incidentData, 'category'));
        setSymptomDistribution(buildDistribution(incidentData, 'symptom'));
        setLocationDistribution(buildDistribution(incidentData, 'location'));
        setImpactDistribution(buildDistribution(incidentData, 'impact'));
        setUrgencyDistribution(buildDistribution(incidentData, 'urgency'));
        setPriorityDistribution(buildDistribution(incidentData, 'priority'));
      } catch (error) {
  ...
  // Function to create pie chart data with top 5 and "Other"
  const createPieChartData = (distribution) => {
    const total = distribution.reduce((sum, item) => sum + item.count, 0);
    const top5 = distribution.slice(0, 5);
    const otherCount = distribution.slice(5).reduce((sum, item) => sum + item.count, 0);

    if (otherCount > 0) {
      top5.push({ value: 'Other', count: otherCount });
    }
  ...
  // Render pie charts for summarized view
  const renderPieCharts = () => {
    const scales = [
      { name: 'Impact', data: createPieChartData(impactDistribution) },
      { name: 'Urgency', data: createPieChartData(urgencyDistribution) },
      { name: 'Priority', data: createPieChartData(priorityDistribution) },
      { name: 'Category', data: createPieChartData(categoryDistribution) },
    ];
  ...
  const PieChart = ({ data, width, height }) => {
    ...
    return (
      <g transform={`translate(${width / 2}, ${height / 2})`}>
        {pie.map((slice, index) => (
          <g key={index}>
            <path d={arc(slice)} fill={color(slice.data.value)} />
            <text
              ...
              fontSize="8px"
            >
              {slice.data.value} ({slice.data.percentage}%)
            </text>
          </g>
        ))}
      </g>
  ...
  // Render the full view when switching back
  useEffect(() => {
    if (viewMode === 'full') {
      ...
      // Generate scales
      const scales = {
        symptom: symptomDistribution.map(d => d.value),
        impact: impactDistribution.map(d => d.value),
        urgency: urgencyDistribution.map(d => d.value),
        priority: priorityDistribution.map(d => d.value),
        location: locationDistribution.map(d => d.value),
        category: categoryDistribution.map(d => d.value),
      };
      ...
      const yHistogramScale = d3.scaleBand()
        .domain(categoryDistribution.map(d => d.value))
      ...
      histogramGroup.selectAll('.bar')
        ...
        .attr('class', 'bar')
        .attr('x', 0)
        .attr('y', d => yHistogramScale(d.value))
      ...
      histogramGroup.selectAll('.total-count')
        ...
        .attr('y', d => yHistogramScale(d.value) + yHistogramScale.bandwidth() / 2)
        .attr('dy', '0.35em')
        .attr('fill', 'white')
        .attr('text-anchor', 'middle')
        .style('font-size', '8px')
  ...
  }, [viewMode, enabledScales, selectedValues, width, height, categoryDistribution, symptomDistribution, locationDistribution, impactDistribution, urgencyDistribution, priorityDistribution, traces, refreshTrigger]);
  ...
  return (
    <div style={{ position: 'relative', minHeight: height }}>
      ...
      <div className="controls" style={{ marginBottom: '10px', textAlign: 'right' }}>
        ...
        >
          {viewMode === 'full' ? 'Show Summarized View' : 'Show Full View'}
        </button>
      </div>
